...
class P1MeterSIM():
    ...
    def __init__(self):
        ...
        self.uart = UART(1, rx=RX_PIN_NR, tx=TX_PIN_NR,
    ...
    async def sender(self):
        ...
        while True:
            log.warning('send simulated telegram on pin {}'.format(TX_PIN_NR))
            ...
            self.telegrams_tx += 1
            await asyncio.sleep(INTERVAL_SIM)
...
async def main():
    asyncio.create_task(P1MeterSIM().sender())
    while True:
        await asyncio.sleep(1)

def run():
    try:
        log.info('mock p1 meter is starting...')
        asyncio.run(main())
    finally:
